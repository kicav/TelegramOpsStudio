...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="TelegramOpsStudio")
    parser.add_argument("--version", action="store_true", help="Print version and exit")
    parser.add_argument("--self-check", action="store_true", help="Run local diagnostics and exit")
    parser.add_argument(
        "--demo-workflow", action="store_true", help="Run a safe fake end-to-end workflow"
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.demo_workflow:
        from telegram_workflow.diagnostics.demo_workflow import run_demo_workflow

        print(run_demo_workflow())
        return 0
